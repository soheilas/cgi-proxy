...
#   those will go.
# If this is not set, it defaults to ".proxy" under your home directory.
# If this is set to a directory name not starting with "/", it will be that
#   directory under your home directory.
# Note that using a directory on a mounted filesystem (which often includes
#   home directories) will prevent that filesystem from being unmounted, which
#   may bother your sysadmin.  If so, try setting this to something starting
#   with "/tmp/", like "/tmp/.yourname".
# To find "your home directory", this program first looks in the HOME
#   environment variable.  If it's not there, it looks in the home directory
#   of the person who owns this program file.  This helps when the Web
#   server runs as a user that's not you.
# If you get "mkdir" permission errors, create the directory yourself-- from your
#   home directory, do "mkdir .proxy" .  You may also need to "chmod 777 .proxy"
#   to make the directory writable by the Web server, but note that this makes
#   it readable and writable by everybody.  You might ask your webmaster if
#   they provide a safe way for the Web server to read and write files in your
#   directories.
# As of version 2.1, this is not actually used yet, since spawn_generic_server()
#   isn't used yet.
$PROXY_DIR= '' ;
...
# Put the script owner's home directory in $ENV{HOME} if that's not set already.
# This is only used for setting $PROXY_DIR.
$ENV{HOME}= (getpwuid( (stat($0))[4] ))[7]  if $ENV{HOME} eq '' ;


# $PROXY_DIR defaults to $ENV{HOME}/.proxy, or under $ENV{HOME} if doesn't
#   start with "/".
$PROXY_DIR= "$ENV{HOME}/$PROXY_DIR"  if $PROXY_DIR=~ /^[^\/]/ ;
$PROXY_DIR= "$ENV{HOME}/.proxy"  if $PROXY_DIR eq '' ;
...
sub http_get {
	...
	print S "X-Requested-With: $ENV{HTTP_X_REQUESTED_WITH}\015\012"
	    if $expected_type eq 'text/xml' and $ENV{HTTP_X_REQUESTED_WITH} eq 'XMLHttpRequest' ;
...
# Tries to listen on $port, but uses any port if that fails.
...
# Returns list of (listening port, daemon PID) on success.  If daemon is already
#   running (as evidenced by lock on $lock_file), reads those values from $lock_file.
...
sub spawn_generic_server {
    ...
    open($LOCK, '+>', "$PROXY_DIR/$lock_file") || die "open: $!" ;
	...
	close($LOCK) ;
	waitpid($pid, 0) ;
	return($port, $?>>8) ;   # $?>>8 is the exit status, which we abuse here
    }

    # Child process continues.

    # Close filehandles in child process.
    # Leave STDERR open to direct error output to the HTTPD error log.
    ...
    # Unconventionally, use exit() status to pass $pid to waiting parent.
    exit($pid) if $pid ;

    # Second child process continues.  This is the daemon process.

    # Record port and PID in lockfile.
    select((select($LOCK), $|=1)[0]) ;   # make $LOCK autoflush output
    print $LOCK "$port,$$\n" ;
    ...
    # Daemon dies if not used for 10 minutes.
    $SIG{ALRM}= sub {exit} ;
    alarm(600) ;
	...
	# Restart 10-minute timer upon each incoming connection.
	alarm(600) ;
...
<table width="100%"><tr>
<td align=left>
<a href="$proxified_homepage"><i>CGIProxy 2.1.1</i></a>
</td>
<td align=right>
$rightlink
</td>
...
$safe_URL2= "http://jmarshall.com/bugs/report.cgi?URL=$safe_URL2&version=2.1.1" ;
...
#		$prefix= $token ;
	...
	#   not ")", then assume it's an object literal.  Not perfect.
	} elsif ($token eq '{' and $term_so_far eq '' 
		 and $last_token=~ /^(?!\))$RE_JS_PUNCTUATOR$/o
...
var _proxy_jslib_handle_properties= 'eval insertAdjacentHTML setAttribute setAttributeNode insertRule innerHTML outerHTML outerText src href background lowsrc action useMap longDesc cite codeBase location poster open write writeln URL referrer baseURI body replace toString String setInterval setTimeout cookie domain frames parent top opener protocol host hostname port pathname search setStringValue setProperty setNamedItem load execScript navigate showModalDialog showModelessDialog execCommand LoadMovie getElementById getElementsByTagName appendChild close'.split(/\s+/) ;

var _proxy_jslib_assign_properties= 'background src href lowsrc action useMap longDesc cite codeBase location poster profile cssText innerHTML outerHTML outerText nodeValue protocol host hostname port pathname search cookie domain value backgroundImage content cursor listStyle listStyleImage text'.split(/\s+/) ;
...
//    if (typeof(o)=='object' && !(property in o)) return void 0 ;
	...
	case 'outerText':
	    ...
	    // jsm-- in HTML5, innerHTML exists on HTMLDocument too.  Handle
	    //   this better when rewrite _proxy_jslib_object_type().
	    if (!(   ('getAttributeNode' in o) && ('getElementsByTagName' in o)
	...
	case 'poster':
	    ...
	    if (!('nodeType' in o) && (otype!='Location'))  return u ;
...
//alert('in Window.top; o, o.name, w, w.name=\n['+o+']['+o.name+']\n['+w+']['+w.name+']') ;  // jsm-- all alerts
	...
	case 'Window.String':
	    return function(s) {
		       var stype= _proxy_jslib_object_type(s);
		       if (stype=='Link' || stype=='Location')
...
//if (o==null) alert('in assign, o is null, property, caller=\n['+property+']\n['+arguments.callee.caller+']') ;   // jsm-- remove in production release?
	...
	case 'background':
	    if ((otype=="CSS2Properties") || (otype=="CSSStyleDeclaration")) {
	...
	case 'poster':
	    ...
	    if (!('nodeType' in o) && (otype!='Location') && (otype!='Window'))
		return eval('o[property]'+op+'val') ;
	    if (opmod!='') {
...
//		case 'CSSStyleDeclaration.background':   // handled above
		case 'CSSStyleDeclaration.backgroundImage':
		case 'CSSStyleDeclaration.content':
		case 'CSSStyleDeclaration.cursor':
		case 'CSSStyleDeclaration.listStyle':
		case 'CSSStyleDeclaration.listStyleImage':
		    o[property]= _proxy_jslib_proxify_css(val) ;
		    return val ;
		...
		case 'Node.text':
			...
			var type= o.type || _proxy_jslib_default_script_type ;
			return o[property]= _proxy_jslib_proxify_block(val, type,
...
// Next two functions for RTMP support.
function _proxy_jslib_full_url_connect(url) {
    var m ;
alert('starting _proxy_jslib_full_url_connect('+url+')') ;
    if (!url) return url ;
    if (url.match(/^http\:\/\//i)) return _proxy_jslib_full_url(url) ;
...
alert('starting _proxy_jslib_full_url_play('+url+')') ;
...
function _proxy_jslib_proxify_js_tokens(jsin, start, end, top_level, with_level, in_new_statement, reverse)
{
  ...
  OUTER:
	...
	} else if (/^(open|write|writeln|replace|load|eval|setInterval|setTimeout|toString|String|src|href|background|lowsrc|action|location|poster|URL|referrer|baseURI|useMap|longDesc|cite|codeBase|profile|cssText|insertRule|setStringValue|setProperty|backgroundImage|content|cursor|listStyleImage|host|hostname|pathname|port|protocol|search|setNamedItem|innerHTML|outerHTML|outerText|body|insertAdjacentHTML|setAttribute|setAttributeNode|nodeValue|value|cookie|domain|frames|parent|top|opener|execScript|execCommand|navigate|showModalDialog|showModelessDialog|LoadMovie|close|getElementById|getElementsByTagName|appendChild|text)$/.test(token)) {
	    _proxy_jslib_needs_jslib= true ;
	...
	//} else if (/^(applets|embeds|forms|ids|layers|anchors|images|links)$/.test(token)) {
		...
		if (term_so_far) {
		   ...
		   && last_token && last_token.match(RE.PUNCDIVPUNC)
...
#warn "in DoAction; out=[".swf2perl($tag)."]\n" ;
	...
	# Handle DoABC tag, including spawning an RTMP proxy
	    ...
	    ($RTMP_SERVER_PORT)= spawn_generic_server(1935, 'rtmp.run', \&rtmp_proxy)
...
#   and NetStream.play() calls.  [Later: added "URLRequest" too.]
sub proxify_swf_cpool_info {
    my($in)= @_ ;
    my(@out, $s) ;
    my $n_specials= { connect => [], play => [], URLRequest => [] } ;
    my $mn_specials= { connect => [], play => [], URLRequest => [] } ;
    ...
    # Copy through strings, adding the 12 strings we need.
    my $string_count= &get_swf_u30_32($in) ;
    push(@out, &set_swf_u30_32($string_count+12)) ;
    $start_pos= pos($$in) ;
    foreach (1..$string_count-1) {
	$s= &get_swf_string($in) ;
	push(@{$n_specials->{'connect'}},  $_) if $s eq 'connect' ;
	push(@{$n_specials->{play}},       $_) if $s eq 'play' ;
	push(@{$n_specials->{URLRequest}}, $_) if $s eq 'URLRequest' ;
    }
    push(@out, substr($$in, $start_pos, pos($$in)-$start_pos)) ;
    push(@out, "\x09flash.net\x0aURLRequest\x03url\x0eflash.external\x11ExternalInterface\x15_proxy_jslib_full_url\x04call\0\x0dNetConnection\x09NetStream\x1d_proxy_jslib_full_url_connect\x1a_proxy_jslib_full_url_play") ;
...
# Besides proxifying the existing method bodies, 3 method bodies must be added.
sub proxify_swf_method_body_infos {
    ...
    my($post_construct_ins, $pre_connect_ins, $pre_play_ins, $pre_coerce_ins_part, $after_jump) ;
	...
	pos($$in)+= $code_length ;
	($code, $insertions)= &proxify_swf_avm2_code($code, $post_construct_ins,
	    $pre_connect_ins, $pre_play_ins, $pre_coerce_ins_part1, $pre_coerce_ins_part2, $mn_specials) ;
...
sub proxify_swf_avm2_code {
    my($code, $post_construct_ins, $pre_connect_ins, $pre_play_ins,
       $pre_coerce_ins_part1, $pre_coerce_ins_part2, $mn_specials)= @_ ;
...
use vars qw($test) ;
	...
	$old_out_len= $out_len ;
&HTMLdie("Bad opcode: [" . swf2perl($op) . "] at position " . pos($code) . " in method body.")
unless defined $AVM2_BYTECODES->[ord($op)] ;    # jsm
	...
	# jsm-- we could handle getproperty(loaderURL) too, to unproxify that URL.
...
# We need to remember which ones use the name "connect", "play", of "URLRequest",
...
sub get_swf_multiname {
	...
	($name_id==$_) && push(@{$mn_specials->{'connect'}}, $mn_id)
	    foreach (@{$n_specials->{'connect'}}) ;
	($name_id==$_) && push(@{$mn_specials->{play}}, $mn_id)
	    foreach (@{$n_specials->{play}}) ;
	($name_id==$_) && push(@{$mn_specials->{URLRequest}}, $mn_id)
	    foreach (@{$n_specials->{URLRequest}}) ;
